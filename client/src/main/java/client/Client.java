...
/**
* Implemented a client class to post to the broker and get the clientApplication instance from the broker
* */
public class Client {
    public static void main(String[] args) {
        RestTemplate restTemplate = new RestTemplate();
        //loop through the clients to post to broker for each client and get the quotations
        for (ClientInfo info: clients) {
            HttpEntity<ClientInfo> request = new HttpEntity<>(info);
            //get the clientApplication instance from broker
            ClientApplication clientApplication = restTemplate.postForObject("http://localhost:8084/applications",
    ...
    public static void displayProfile(ClientInfo info) {
        ...
        System.out.println(
                        "| Name: " + String.format("%1$-29s", info.getName()) +
                                        " | Gender: " + String.format("%1$-27s", (info.getGender()==ClientInfo.MALE?"Male"
                                        :"Female")) +
